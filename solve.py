...
class Solver:
    ...
    def _findClosestColor(self, color, memorize=False):
        ...
        if memorize == True: self.memorized_color_map[color] = closest_color
    ...
    def _findColoredStart(self):
        logging.info("Finding START point based on a color...")
        start = self._findColorCenter(self.START_COLOR)
        self._drawSquare(start, self.START_COLOR)
        return start

    def _findColoredEnd(self):
        logging.info("Finding END point based on a color...")
        end = self._findColorCenter(self.END_COLOR)
        self._drawSquare(end, self.END_COLOR)
        return end
    
    def _findStart(self):
        logging.info("Finding a START point and Color it with GREEN")
        start = self._findStart(self.start)
        return start

    def _findEnd(self):
        logging.info("Finding an END point and color it with RED")
        end = 
        return end
